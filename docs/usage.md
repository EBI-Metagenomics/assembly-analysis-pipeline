# ebi-metagenomics/assembly-analysis-pipeline: Usage

> [!NOTE]
> This pipeline follows some of the conventions and uses tools from the nf-core community, but it's not part of the nf-core project.
...
| `contaminant_reference` | Name of the contaminant reference genome subfolder. Folder must contain FASTA file with the contaminant genome sequence. **Optional**                                                  |
...
```bash
nextflow run ebi-metagenomics/assembly-analysis-pipeline --input ./samplesheet.csv --outdir ./results  -profile docker
```
...
```bash
nextflow run ebi-metagenomics/assembly-analysis-pipeline -profile docker -params-file params.yaml
```
...
```bash
nextflow pull ebi-metagenomics/assembly-analysis-pipeline
```
...
First, go to the [ebi-metagenomics/assembly-analysis-pipeline releases page](https://github.com/ebi-metagenomics/assembly-analysis-pipeline/releases) and find the latest pipeline version - numeric only (eg. `6.0.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 6.0.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
...
In some cases, you may wish to change the container or conda environment used by a pipeline steps for a particular tool. By default, this pipeline use containers and software from the [biocontainers](https://biocontainers.pro/) or [bioconda](https://bioconda.github.io/) projects. However, in some cases the pipeline specified version maybe out of date.
